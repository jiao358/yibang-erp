...
-- 1. 为现有表添加新字段（如果不存在）
-- 为roles表添加supplier_scope字段
ALTER TABLE roles 
ADD COLUMN IF NOT EXISTS supplier_scope ENUM('ALL', 'OWN_SUPPLIER', 'OWN_SALES') DEFAULT 'OWN_SALES' COMMENT '权限范围：ALL-全部，OWN_SUPPLIER-仅自己供应链，OWN_SALES-仅自己销售' AFTER permissions;

-- 为companies表添加供应链相关字段
ALTER TABLE companies 
ADD COLUMN IF NOT EXISTS parent_supplier_id BIGINT COMMENT '上级供应链公司ID（可为空，表示顶级供应链公司）' AFTER type,
ADD COLUMN IF NOT EXISTS supplier_level INT DEFAULT 1 COMMENT '供应链层级：1-顶级，2-二级，3-三级' AFTER parent_supplier_id;

-- 为users表添加多租户关联字段
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS supplier_company_id BIGINT COMMENT '所属供应链公司ID（可为空，表示个人销售）' AFTER company_id,
ADD COLUMN IF NOT EXISTS sales_company_id BIGINT COMMENT '所属销售公司ID（可为空，表示个人销售）' AFTER supplier_company_id,
ADD COLUMN IF NOT EXISTS sales_type ENUM('COMPANY_SALES', 'INDIVIDUAL_SALES') DEFAULT 'INDIVIDUAL_SALES' COMMENT '销售类型：COMPANY_SALES-公司销售，INDIVIDUAL_SALES-个人销售' AFTER sales_company_id;
...
-- 为users表添加外键约束
ALTER TABLE users 
ADD CONSTRAINT IF NOT EXISTS fk_user_supplier_company 
FOREIGN KEY (supplier_company_id) REFERENCES companies(id);

ALTER TABLE users 
ADD CONSTRAINT IF NOT EXISTS fk_user_sales_company 
FOREIGN KEY (sales_company_id) REFERENCES companies(id);

-- 为companies表添加外键约束
ALTER TABLE companies 
ADD CONSTRAINT IF NOT EXISTS fk_company_parent_supplier 
FOREIGN KEY (parent_supplier_id) REFERENCES companies(id);

-- 8. 创建索引（如果不存在）
-- 为users表添加索引
CREATE INDEX IF NOT EXISTS idx_supplier_company ON users(supplier_company_id);
CREATE INDEX IF NOT EXISTS idx_sales_company ON users(sales_company_id);
CREATE INDEX IF NOT EXISTS idx_sales_type ON users(sales_type);
CREATE INDEX IF NOT EXISTS idx_user_supplier_sales ON users(supplier_company_id, sales_company_id, sales_type);

-- 为companies表添加索引
CREATE INDEX IF NOT EXISTS idx_parent_supplier ON companies(parent_supplier_id);
CREATE INDEX IF NOT EXISTS idx_supplier_level ON companies(supplier_level);

-- 为roles表添加索引
CREATE INDEX IF NOT EXISTS idx_supplier_scope ON roles(supplier_scope);

-- 为company_supplier_relationships表添加索引
CREATE INDEX IF NOT EXISTS idx_relationship_supplier_status ON company_supplier_relationships(supplier_company_id, status);
CREATE INDEX IF NOT EXISTS idx_relationship_sales_status ON company_supplier_relationships(sales_company_id, status);
